...
lv_disp_t *lvgl_port_add_disp(const lvgl_port_display_cfg_t *disp_cfg)
{
    ...
    if (disp_ctx->trans_size) {
        ...
        /* One token means no DMA transfer is currently reading a transport
         * buffer. The completion callback returns the token. */
    ...
    /* The AXS15231B QSPI write path streams a complete frame. */
...
static void lvgl_port_flush_callback(lv_disp_drv_t *drv, const lv_area_t *area, lv_color_t *color_map)
{
    ...
    if (disp_ctx->trans_size) {
        ...
        for (int i = 0; i < trans_count; i++) {
            ...
            if (i == 0 && disp_ctx->draw_wait_cb) {
                disp_ctx->draw_wait_cb(disp_ctx->panel_handle->user_data);
            }

            /* Wait before touching the next transport buffer. Previously this
             * happened after the copy, allowing LVGL's next frame to overwrite
             * pixels while GDMA was still sending the previous frame. */
            ...
            case LV_DISP_ROT_180:
                for (int y = 0; y < trans_height; y++) {
                    for (int x = 0; x < width; x++) {
                        *(to + (trans_height - y - 1)*width + (width - x - 1)) = *(from + (y_start_tmp - y_start + y) * width + x);
                    }
                }
            ...
            case LV_DISP_ROT_NONE:
                for (int y = 0; y < trans_height; y++) {
                    for (int x = 0; x < width; x++) {
                        *(to + y * width + x) = *(from + (y_start_tmp - y_start + y) * width + x);
                    }
                }
